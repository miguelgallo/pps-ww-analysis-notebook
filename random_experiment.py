...
def random_experiment( df, variables, variations, lumi_weights=None, data_periods=None, resample=None ):
    
    probs_lumi_ = None
    if lumi_weights is not None:
    ...
    for var_ in variables:
        ...
        if probs_lumi_ is not None:
            ...
            df_arr_period_ = df.loc[ :, "period" ]
            df__ = pd.DataFrame( np.c_[ df_arr_period_, df_arr_Arm_, df_arr_var_ ], columns=("period","Arm",var_) )
        print ( df__ )
