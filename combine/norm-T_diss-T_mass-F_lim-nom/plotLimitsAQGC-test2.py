...
#parser.add_argument(
#    'input2', nargs='+', help="""Input json files""")
parser.add_argument(
...
parser.add_argument(
    '--x-title', default='a^{0}_{W}/#Lambda^{2} (#times 10^{-7} GeV^{-2})', help="""Title for the x-axis""")
parser.add_argument(
    '--y-title', default=None, help="""Title for the y-axis""")
parser.add_argument(
    '--limit-on', default='#sigma/#sigma_{AQGC}', help="""Shortcut for setting the y-axis label""")
parser.add_argument(
...
parser.add_argument(
    ...
    left of plot""")
parser.add_argument(
    '--title-right', default='55.7 fb^{-1}, 2018 (13 TeV)', help="""Right header text above the frame""")
parser.add_argument(
    '--title-left', default='Work in Progress', help="""Left header text above the frame""")
parser.add_argument(
...
#def DrawAxisHist(pads, axis_hists, def_pad=None):
#    for i, pad in enumerate(pads):
#        pad.cd()
#        axis_hists.Draw('AXIS')
#        axis_hists.Draw('AXIGSAME')
#    if def_pad is not None:
#        def_pad.cd()
...
for src in args.input:
    ...
    # as before but also apply style options to TGraph
        ...
        graphs.append(plot.LimitTGraphFromJSONFile(file, splitsrc[0]))
        if len(splitsrc) >= 3:
