...
#   Generated: Sun Jan 10 23:08:21 2021
...
if ( -e $thisdir/scripts/gaudirun.py ) then
    set Gaudi_DIR=`readlink $thisdir/scripts/gaudirun.py`
    set Gaudi_DIR=`dirname ${Gaudi_DIR}`
    set Gaudi_DIR=`dirname ${Gaudi_DIR}`
    setenv CMAKE_PREFIX_PATH ${Gaudi_DIR}:${CMAKE_PREFIX_PATH}
endif
...
if ( -e $thisdir/bin/geant4-config ) then
    ...
    setenv ROOT_INCLUDE_PATH ${thisdir}/include/Geant4:${ROOT_INCLUDE_PATH}
endif
...
if ( -e $thisdir/lib/libopenloops.so ) then
    ...
    setenv OpenLoopsPath $proclib_path
endif
...
#--- CLHEP
if ( -e $thisdir/bin/clhep-config ) then
    set __dir=`$thisdir/bin/clhep-config --version | sed s/' '/-/g`
    if ( -d $thisdir/lib/libCLHEP.so ) then
       set clhep_dir=`readlink $thisdir/lib/libCLHEP.so`
       set clhep_dir=`dirname $clhep_dir`
       setenv CLHEP_DIR $clhep_dir/$__dir
    endif
endif

#---then DD4hep
if ( -x $thisdir/bin/ddsim ) then
   set dd4hepdir=`readlink $thisdir/bin/ddsim`
   set dd4hepdir=`dirname ${dd4hepdir}`
   set dd4hepdir=`dirname ${dd4hepdir}`
   setenv DD4hepINSTALL ${dd4hepdir}
endif
